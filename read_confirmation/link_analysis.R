...
all_tbl %>>%filter(is.na(screening),mutect_dcv_posi/mutect_mut_num>0.1,mutect_dcv_posi/mutect_mut_num<0.9)%>>%
...
ratio=all_tbl %>>%
  ...
  ggplot()+geom_bar(aes(x=distance_class,fill=linked_convert),color="black",position = "fill",width = 0.95)+
  geom_signif(stat = "identity",aes(x=2,xend=3,y=1.1,yend=1.1,annotation="**"))+
